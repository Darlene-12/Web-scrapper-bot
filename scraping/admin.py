...
class ScrapingScheduleAdmin(admin.ModelAdmin):
    list_display = ['name', 'url_display',  'frequency', 'is_active', 'last_run', 'next_run']
    ...
    fieldsets = (
        ...
        ('Schedule', {'fields': ('frequency', 'cron_expression', 'is_active')}),
        ('Timing', {'fields': ('created_at', 'last_run', 'next_run')}),
        ('Execution Parameters', {'fields': ('use_selenium', 'custom_headers', 'timeout')}),
        ('Notifications', {'fields': ('notify_on_completion', 'notification_email')}),
    )
    ...
    def run_now(self, request, queryset):
        ...
        for schedule in queryset:
            task = scrape_url_task.delay(schedule.url, schedule.keywords, schedule.data_type,
                                         schedule.use_selenium, schedule.custom_headers, schedule.timeout)
            self.message_user(request, f"Scheduled immediate execution for '{schedule.name}' (Task ID: {task.id})")
...
class ScrapingProxyAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['last_used', 'success_count', 'failure_count', 'average_response_time']
    actions = ['activate_proxies', 'deactivate_proxies', 'reset_counters']

    fieldsets = (
        ('Proxy Details', {'fields': ('address', 'port', 'proxy_type', 'is_active')}),
    ...
    def success_rate_display(self, obj):
        return f"{obj.success_rate:.1f}%" if obj.success_rate else "-"
